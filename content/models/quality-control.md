...
sourceUrl: "https://mungermodels.com/models/quality-control"
---
...
这个数字看起来极端苛刻，但它反映了一个深刻的工程学直觉：**在[复杂系统](/model/complex-systems)中，每个组件的微小缺陷会通过乘法效应放大为整体的显著缺陷。** 一辆汽车有数万个零件。如果每个零件的缺陷率是千分之一（三西格玛水平），那一辆车上出现至少一个有缺陷零件的概率几乎是100%。你必须把每个零件的缺陷率压到百万分之几，才能让最终产品有合理的可靠性。
...
他有一套严格的投资检查流程：这家企业的商业模式我是否真正理解？它的[护城河（Moat）](/model/moat)是否可持续？管理层是否诚信且能干？价格是否提供了足够的[安全边际](/model/margin-of-safety)？有没有我遗漏的重大风险？每一个问题都是生产线上的一个“质量检查站”。任何一个问题的答案如果不够令人满意，整个“生产线”就停下来——不买。就像丰田工人拉动安灯绳一样。
...
德明教导日本企业家的核心是“减少变异”。在投资中，“变异”对应的是决策质量的不稳定——有时候你做了充分的研究，有时候你凭直觉冲动行事；有时候你冷静理性，有时候你被恐惧或贪婪驱动；有时候你在[能力圈](/model/circle-of-competence)内行动，有时候你冒险进入自己不懂的领域。

这种决策质量的“变异”是投资者最大的敌人之一。芒格通过建立一套系统化的决策流程来控制它：检查清单、[逆向思维](/model/inversion)（先考虑什么会让投资失败）、强制等待期（防止冲动决策）、与巴菲特的交叉验证。这些机制就像德明的控制图——它们不能保证每个决策都正确，但它们把决策质量的“变异”控制在一个可接受的范围内。
...
可口可乐几十年如一日地保持产品配方和品质的一致性。苹果对产品的质量标准苛刻到了令供应商恐惧的地步。这些企业的共同特征是：它们把质量控制内化成了组织的DNA，而不是靠终端检验来亡羊补牢。芒格投资这样的企业，本质上是在投资“低变异”系统——这类系统的长期表现更可预测，更不容易突然触及[断裂点](/model/breakpoints)。
...
这正是他多元思维模型体系的核心信念。你不需要掌握一千个模型。你需要真正掌握几十个最基本的模型——来自数学、物理、生物、工程学、心理学——然后把它们交叉应用于你遇到的每一个问题。质量控制的成功，就是[概率思维与期望值](/model/probabilistic-thinking-expected-value)和[大数定律](/model/law-of-large-numbers)在工业中的交叉应用。投资中的[检查清单方法](/model/checklist-method)，就是质量控制思维和心理学（对抗[避免怀疑倾向](/model/doubt-avoidance-tendency)和[自视过高的倾向](/model/excessive-self-regard-tendency)）的交叉应用。
...
- [概率思维与期望值](/model/probabilistic-thinking-expected-value) — 质量控制的数学基础：概率的乘法法则直接决定了多环节系统的整体缺陷率
- [大数定律](/model/law-of-large-numbers) — 质量控制的统计基石：大量重复生产才能让统计规律显现
- [复利效应](/model/compound-interest) — 质量改进的复利：持续的微小改善在长期产生指数级的效果
- [安全边际](/model/margin-of-safety) — 质量控制在生产中的作用类似安全边际在投资中的作用：为不可避免的变异预留缓冲
- [冗余备份系统](/model/redundancy) — 冗余是硬件层面的质量保障，质量控制是过程层面的质量保障
- [断裂点](/model/breakpoints) — 低质量（高变异）的系统更容易触及断裂点
- [护城河（Moat）](/model/moat) — 卓越的质量控制文化本身就是一条强大的护城河
- [避免怀疑倾向](/model/doubt-avoidance-tendency) — 检查清单对抗“匆忙下结论”的心理倾向
- [自视过高的倾向](/model/excessive-self-regard-tendency) — 质量控制强迫你承认“我的判断可能有误”，对抗过度自信
- [迭代与原型](/model/iteration-prototyping) — 快速迭代与原型测试是质量控制的前置环节：在早期发现问题远比后期修复成本低
