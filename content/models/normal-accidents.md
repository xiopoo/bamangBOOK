...
sourceUrl: "https://mungermodels.com/models/normal-accidents"
---
...
反应堆压力升高，泄压阀自动打开。压力回到正常后，泄压阀本应自动关闭——但它卡住了。控制面板上有一个指示灯显示“泄压阀关闭指令已发出”——操作员看到了这个灯，认为阀门已经关闭。但这个灯显示的是**指令是否发出**，不是**阀门是否实际关闭**。
...
**第三个反直觉：经验丰富的操作员可能比新手更危险。** 经验丰富的人因为过度自信于自己的心智模型，更难接受“系统正在以我从未见过的方式失败”这个事实。这和芒格强调的[避免不一致性倾向](/model/inconsistency-avoidance-tendency)高度吻合。
...
1. **评估系统耦合度和复杂性。** 一个行业或企业处在佩罗矩阵的哪个象限？复杂+紧耦合环境中的企业面临的不是“事故可能发生”的风险，而是“事故迟早发生”的确定性。你的[安全边际](/model/margin-of-safety)需要为此留足空间。
...
芒格一辈子都在做这种选择。他避开自己不理解的[复杂系统](/model/complex-systems)。他把伯克希尔设计成松耦合的简单结构。他保持大量现金作为缓冲。他拒绝杠杆——因为杠杆是紧耦合的极端形式。
...
- [紧耦合与松耦合](/model/tight-coupling-loose-coupling) — 耦合度是正常事故理论的核心维度之一
- [容错设计与优雅降级](/model/fault-tolerance) — 容错设计是应对正常事故风险的工程策略
- [单点故障](/model/single-point-of-failure) — 复杂系统中的单点故障是正常事故的常见触发点
- [断裂点](/model/breakpoints) — 紧耦合系统一旦到达断裂点，恢复极为困难
- [故障模式分析](/model/failure-mode-analysis) — 系统化识别潜在故障模式，是降低正常事故概率的方法
- [安全边际](/model/margin-of-safety) — 为“不可避免的意外”预留缓冲
- [能力圈](/model/circle-of-competence) — 不进入自己无法理解的复杂系统，是避免正常事故的芒格策略
- [复杂适应系统](/model/complex-adaptive-systems) — 理解系统复杂性的更广泛框架
- [脆弱性与反脆弱性](/model/fragility-antifragility) — 正常事故描述了脆弱系统的必然命运，反脆弱是结构性回应
- [涌现性](/model/emergence) — 正常事故本质上是一种负面涌现
- [逆向思维](/model/inversion) — “先假设会失败，然后倒推失败路径”是正常事故理论的决策应用
- [检查清单方法](/model/checklist-method) — 检查清单在复杂系统中降低人为失误概率，但无法消除正常事故
- [自组织临界性](/model/self-organized-criticality) — 两个理论都指向同一结论：复杂系统中的灾难是内生的
