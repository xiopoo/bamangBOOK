...
sourceUrl: "https://mungermodels.com/models/law-of-large-numbers"
---
...
三个季度的时间里，噪声的幅度可能远大于信号。一个平庸的CEO碰上顺风行业，业绩可以看起来非常漂亮。一个优秀的CEO碰上行业寒冬，业绩可能一塌糊涂。你需要**足够多的“试验次数”**——足够多的季度、足够多的商业周期——才能让信号从噪声中浮现出来。
...
投资作家纳西姆·塔勒布（Nassim Taleb）在《随机致富的傻瓜》中讲了一个[思想实验](/model/thought-experiments)：如果把10000只猴子放在打字机前随机敲键盘，其中大概率会有一只敲出一首莎士比亚的十四行诗。你会因此认为那只猴子是文学天才吗？
...
不。轮盘没有记忆。下一次出红色和出黑色的概率和前八次完全无关。大数定律说的是“在无限次试验后，红色和黑色的比例会趋近50:50”，但它**从未说过“短期内的偏差一定会被纠正”**。大数定律通过增加新的试验来稀释偏差，而不是通过让后续结果“补偿”前面的偏差。
...
**边界二：大数定律不适用于一次性事件。** 你只结一次婚、只选一次大学、只活一辈子。这些决策没有“大量重复试验”的空间。大数定律在这里的意义不是直接适用，而是提醒你：**对于不可重复的重要决策，你不能依赖概率的长期均值来保护自己——你必须格外关注单次结果的分布和最坏情况。** 这和[安全边际](/model/margin-of-safety)的逻辑直接相连。
...
- **[贝叶斯定理](/model/bayes-theorem)** — 先验概率的可靠性取决于支持它的样本量
- **[决策树理论](/model/decision-tree-theory)** — 决策树中的概率估计在大样本下更可靠
- **[回归均值](/model/regression-to-the-mean)** — 大数定律的直接推论：极端表现倾向于回归平均水平
- **[复利效应](/model/compound-interest)** — 大数定律保证了微弱但持续的优势会在长期中指数放大
- **[安全边际](/model/margin-of-safety)** — 对于不可重复的单次决策，大数定律无法保护你，安全边际是替代方案
- **[自视过高的倾向](/model/excessive-self-regard-tendency)** — 人们把小样本中的好运归因于自己的能力
- **[社会认同倾向](/model/social-proof-tendency)** — 大众容易被短期业绩的“明星”带偏
- **[奖励和惩罚超级反应倾向](/model/reward-and-punishment-superresponse-tendency)** — 短期业绩[激励机制](/model/incentives-economic-view)与大数定律的要求直接冲突
...
- [ ] 是不是“[幸存者偏差](/model/survivorship-bias)”？你只看到了成功案例，而忽略了同期失败的大量案例
...
- [ ] 如果这是一次不可重复的重要决策（没有“大样本”的保护），我是否为最坏情况留了足够的[安全边际](/model/margin-of-safety)？
