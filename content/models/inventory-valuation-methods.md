...
sourceUrl: "https://mungermodels.com/models/inventory-valuation-methods"
---
...
事实上，很多聪明的企业确实选择了LIFO——但原因不仅仅是利润表更准确，更核心的驱动力是**税收**。
...
芒格强调的[跨学科思维](/model/multidisciplinary-approach)在这里体现得淋漓尽致。表面上这是一个会计问题，但它实质上是一个认识论问题：**你以为你在比较两家企业的经营能力，但你实际上在比较两种不同的计量规则。** 如果你不理解计量规则的差异，你就无法从数字中提取真正的信息。
...
不是。FIFO和LIFO描述的是**成本的流转假设**，而非货物的物理流转。一家面包店可能实际上总是先卖出最新鲜的面包（物理上是后进先出），但在会计上使用FIFO方法。物理流转和成本流转是两个完全独立的概念。
...
- [会计作为商业语言及其局限](/model/accounting-as-language-its-limits) — 存货计价方法是会计语言固有主观性的典型例证
- [现金流量vs利润](/model/cash-flow-vs-earnings) — 无论选择FIFO还是LIFO，现金流出是一样的；利润的差异只是会计幻觉
- [折旧与摊销](/model/depreciation-amortization) — 折旧方法的选择与存货计价方法的选择是同一类问题：管理层假设驱动的利润差异
- [复式记账法](/model/double-entry-bookkeeping) — 理解存货如何同时影响利润表（销售成本）和资产负债表（存货项目）
- [激励机制](/model/incentives-economic-view) — 管理层选择FIFO还是LIFO，背后往往有薪酬激励和税收动机的驱动
- [逆向思维](/model/inversion) — 看到利润增长时，逆向追问：有多少是存货利润而非经营利润？
