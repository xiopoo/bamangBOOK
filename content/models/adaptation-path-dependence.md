...
sourceUrl: "https://mungermodels.com/models/adaptation-path-dependence"
---
...
工程学视角的路径依赖（见[路径依赖与锁定](/model/path-dependence-lock-in)）聚焦于技术标准如何被锁定。但从[复杂系统](/model/complex-systems)科学的角度看，路径依赖是一个更深层的现象——它是[复杂适应系统](/model/complex-adaptive-systems)（Complex Adaptive Systems, CAS）的固有特征。

复杂适应系统有三个关键属性：多个相互作用的智能体（agents）、这些智能体会根据环境反馈调整自己的行为（adaptation）、以及系统整体会[涌现](/model/emergence)出智能体个体所不具有的宏观模式。经济体、生态系统、城市、大脑——都是复杂适应系统。
...
但阿瑟指出，在知识密集型经济中——软件、制药、高科技——回报往往是递增的。开发第一份Windows拷贝花了数十亿美元，复制第二份的边际成本接近零。用户越多，软件越有价值（[网络效应](/model/network-effects)）。学习曲线越长，切换成本越高。在递增回报的世界中，系统不会趋向唯一的均衡——它会趋向多个可能的均衡中的某一个，而最终“选中”哪个均衡，取决于历史路径中的偶然事件。
...
键盘布局的QWERTY故事已经在工程学篇中详述（见[路径依赖与锁定](/model/path-dependence-lock-in)），但从系统层面看，它的意义更为深远。QWERTY不仅仅是一个“次优标准被锁定”的故事——它揭示了一个普遍的系统动力学：**早期的微小优势可以通过正反馈被放大为不可逾越的壁垒，即使那个“优势”早已不复存在。**
...
芒格对这些现象的理解深刻地影响了他的投资判断。他寻找的不是“今天最好的产品”，而是**已经建立了路径依赖护城河的企业**——那些通过递增回报和正反馈循环锁定了客户、供应商和整个生态系统的公司。一旦路径被锁定，竞争对手需要的不只是“更好的产品”，而是需要足够强大的力量来打破整个系统的惯性。这种力量极其罕见。

---

## 适应性的悖论：为什么昨天的最优适应是今天的牢笼

路径依赖有一个更微妙的维度，它涉及系统的[适应性](/model/adaptation)本身。
...
**边界条件一：路径依赖不意味着一切不可改变。** 虽然路径[转换成本](/model/switching-costs)高昂，但并非不可能。技术范式的转变（从马车到汽车、从胶片到数码）证明路径可以被打破——但通常需要一个足够强大的外部冲击或全新的技术平台来“重置”正反馈循环。
...
- [路径依赖与锁定](/model/path-dependence-lock-in) — 工程学视角的路径依赖，聚焦技术标准锁定
- [涌现性](/model/emergence) — 路径依赖的系统行为是从个体互动中涌现出来的
- [网络理论](/model/network-theory) — 网络效应是驱动路径依赖的最强正反馈机制之一
- [飞轮效应](/model/flywheel-effect) — 飞轮的自我强化循环是递增回报的商业表达
- [沉没成本谬误](/model/sunk-cost-fallacy) — 路径依赖中的“锁定”部分源于沉没成本的心理效应
- [规模效应与反效应](/model/scaling-effects-diseconomies-in-engineering) — 规模递增回报是路径依赖的经济学基础
- [自组织临界性](/model/self-organized-criticality) — 系统在临界态的演化路径是不可逆且不可预测的
- [脆弱性与反脆弱性](/model/fragility-antifragility) — 过度适应当前路径是脆弱性的来源
