...
sourceUrl: "https://mungermodels.com/models/multiplicative-systems"
---
...
芒格对[复杂系统](/model/complex-systems)的怀疑态度——他偏好简单、确定、环节少的投资——正是建立在这个数学基础之上的。他不是因为“保守”才回避复杂性，而是因为他算过账。
...
**边界条件：乘法法则假设“一个环节失败则整体失败”。** 但现实中很多系统有容错能力——某个环节出了小问题可以修复、可以绕过、可以用替代方案弥补。工程学中的[冗余备份系统](/model/redundancy)就是专门用来打破乘法链条的——如果关键环节有两套并联的备份，那么这个环节的失败率就从P变成了P^2，极大地提高了可靠性。芒格在生活中也使用冗余策略：他不只依赖一种收入来源，不只信任一种分析方法，不把所有资产集中在一个地方。
...
- **[概率思维与期望值](/model/probabilistic-thinking-expected-value)** — 乘法系统思维是概率思维在串联结构中的特殊应用
- **[排列组合原理](/model/permutations-and-combinations)** — 排列组合提供了计算串联和并联概率的数学工具
- **[冗余备份系统](/model/redundancy)** — 冗余是对抗乘法衰减的核心工程策略：用并联替代串联
- **[安全边际](/model/margin-of-safety)** — 安全边际为乘法链条中的估算误差提供缓冲
- **[能力圈](/model/circle-of-competence)** — 芒格的能力圈原则本质上是选择环节更少、确定性更高的投资
- **[复利效应](/model/compound-interest)** — 复利是乘法法则的正面应用——持续正回报的乘积产生指数增长
- **[断裂点](/model/breakpoints)** — 乘法链条中最弱的环节就是系统的潜在断裂点
- **[护城河（Moat）](/model/moat)** — 宽阔的护城河减少了企业成功所需的“必须做对的事情”的数量
