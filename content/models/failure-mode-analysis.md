...
sourceUrl: "https://mungermodels.com/models/failure-mode-analysis"
---
...
这四步的核心逻辑可以浓缩为一个公式：**[风险优先](/model/risk-first)数（RPN）= 严重性 × 发生概率 × 不可检测性。** RPN最高的故障模式，就是你应该首先解决的问题。
...
他最典型的做法是所谓的“[逆向思维](/model/inversion)”——在考虑一笔投资时，不是先问“这家企业为什么会成功”，而是先问“什么会杀死这家企业”。这就是FMEA在投资中的直接应用：系统性地列举每一种可能导致企业价值归零的路径。
...
**管理层[道德风险](/model/moral-hazard)。** 管理层是否有动机和能力通过损害股东利益来谋取私利？这是芒格极为看重的故障模式。他说过：“给我看[激励机制](/model/incentives-economic-view)，我就能告诉你结果。”如果管理层的薪酬结构鼓励短期行为，那么无论财务报表多么漂亮，管理层道德风险这一故障模式的发生概率就很高。
...
**第四个局限：故障之间的交互作用。** 标准FMEA逐项分析每种故障模式，但现实中最致命的灾难往往来自多个小故障的交互叠加。每个单独的故障都在“可接受”范围内，但它们同时发生时就产生了灾难性的组合效应。社会学家Charles Perrow把这种现象称为“[正常事故](/model/normal-accidents)”（normal accident）——在足够复杂的系统中，多重小故障的巧合叠加几乎是统计上的必然。
...
- [逆向思维](/model/inversion) — FMEA是逆向思维最系统化的实践形式：从“什么会出错”出发来设计解决方案
- [冗余备份系统](/model/redundancy) — FMEA识别出关键故障模式后，冗余设计是最常见的应对手段
- [单点故障](/model/single-point-of-failure) — FMEA经常揭示出系统中的单点故障，进而推动冗余设计
- [安全边际](/model/margin-of-safety) — 安全边际是投资领域对已知故障模式的预留缓冲
- [概率思维与期望值](/model/probabilistic-thinking-expected-value) — FMEA中的“严重性×概率”本质上是期望值计算
- [避免痛苦的心理否认](/model/simple-pain-avoiding-psychological-denial) — 挑战者号案例的核心教训：心理否认会瓦解最严谨的故障分析
- [检查清单方法](/model/checklist-method) — 检查清单是FMEA成果的日常化应用形式
- [断裂点](/model/breakpoints) — FMEA试图在系统到达断裂点之前识别并消除隐患
- [权衡分析](/model/trade-off-analysis) — FMEA识别故障后，权衡分析帮助决定在可靠性、成本和性能之间如何取舍
