...
sourceUrl: "https://mungermodels.com/models/regression-to-the-mean"
---
...
不会。因为回归均值只描述了**条件期望**——给定父母是极端值，子女的期望值更接近均值。但身高的整体分布并没有在缩窄。每一代中，都有一些人因为遗传和环境因素的幸运组合而成为新的极端值。分布的形状保持稳定，但具体哪些家庭处于极端位置在不断洗牌。
...
巴菲特和芒格的投资框架可以用回归均值来理解。他们寻找的“[护城河（Moat）](/model/moat)”，本质上就是**抵抗回归均值的力量**。品牌、[网络效应](/model/network-effects)、专利、[转换成本](/model/switching-costs)、规模经济——这些竞争优势的作用不是创造超额利润，而是保护超额利润不被竞争侵蚀，延缓甚至阻止回归均值的发生。
...
逆向投资的核心逻辑是：买入被市场过度悲观定价的资产，卖出被市场过度乐观定价的资产。为什么这种策略长期来看往往有效？因为**市场的极端情绪——无论是狂热还是恐慌——本质上就是一种“极端值”，而极端值倾向于回归均值。**

2009年3月，全球金融危机的最低点，市场的恐慌程度达到了历史极端。人们相信金融体系将永久崩溃，银行股几乎一文不值，连蓝筹股都被抛售到了荒谬的价格。在那个时刻买入，需要对抗巨大的心理压力。但回归均值的逻辑非常清晰：这种程度的恐慌是不可持续的。情绪会回归，估值会回归，价格终将回归到更接近[内在价值](/model/intrinsic-value)的水平。
...
区分这两种情况，是逆向投资者最核心的能力。而这种区分能力，需要对企业和行业的深度理解——这就回到了芒格反复强调的[能力圈](/model/circle-of-competence)概念。
...
**边界四：[幸存者偏差](/model/survivorship-bias)会伪装回归均值。** 如果你只观察“活下来的”样本，你可能误判回归均值的方向。那些业绩从优秀“回归”到平庸的基金经理你能看到；那些业绩从平庸“回归”到破产的基金经理你看不到——因为他们消失了。在有幸存者偏差的数据中，回归均值的真实幅度可能比你观察到的更大。
...
**第五，区分“回归均值”和“趋势逆转”。** 这是最难的部分。股价从高点下跌10%，可能是回归均值（此前涨多了），也可能是一个下行趋势的开始（基本面恶化了）。你需要其他思维模型——[护城河（Moat）](/model/moat)分析、[贝叶斯定理](/model/bayes-theorem)的证据更新、对行业结构的理解——来做这个区分。回归均值本身不告诉你答案，它只告诉你一个统计上的默认预期。
...
- **[大数定律](/model/law-of-large-numbers)** — 回归均值是大数定律的直接推论：样本越大，极端值的影响越被稀释
- **[正态与非正态分布](/model/normal-non-normal-distributions)** — 正态分布解释了为什么极端值稀有且不可持续
- **[护城河（Moat）](/model/moat)** — 护城河是企业对抗利润率回归均值的防御工事
- **[安全边际](/model/margin-of-safety)** — 安全边际是投资者利用回归均值（买入被低估的资产）的操作框架
- **[概率思维与期望值](/model/probabilistic-thinking-expected-value)** — 回归均值提醒你：单次极端结果的信息含量远低于长期期望值
- **[贝叶斯定理](/model/bayes-theorem)** — 极端表现之后，[贝叶斯更新](/model/bayesian-updating)应该将你的信念向均值方向调整
- **[自视过高的倾向](/model/excessive-self-regard-tendency)** — 人们把回归均值中的好运归因于自己的能力
- **[避免不一致性倾向](/model/inconsistency-avoidance-tendency)** — 即使证据指向回归均值，人们也不愿放弃“因果叙事”
- **[进化论](/model/modern-darwinian-synthesis)** — 竞争导致的利润率回归均值，是经济领域的自然选择
- **[逆向思维](/model/inversion)** — 逆向投资的统计学基础正是回归均值
- **[Lollapalooza倾向](/model/lollapalooza-tendency)** — 多种极端因素叠加后的回归可能同样极端
- **[临界质量与相变](/model/critical-mass-phase-transitions-in-systems)** — 结构性变化可以永久改变“均值”本身，使回归失效
- **[网络效应](/model/network-effects)** — 处于正反馈循环中的网络效应，在达到[临界质量](/model/critical-mass)前可能抵御回归
