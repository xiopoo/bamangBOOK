...
scenarios:
  - "企业竞争力与商业模式分析"
  - "商业创新与竞争战略"
description: "为什么明明 Dvorak 键盘更快，全世界还在用 QWERTY？路径依赖与锁定效应：一次偶然选择如何通过网络效应和互补资产被锁死成不可逆的标准。看穿技术选择、平台战争、个人路径的真实机制。"
source: "mungermodels.com"
sourceUrl: "https://mungermodels.com/models/path-dependence-lock-in"
---
...
这不是因为QWERTY更好。1873年克里斯托弗·肖尔斯设计这个布局时，目的甚至不是为了提高打字效率——恰恰相反，他是为了**降低**打字速度，因为当时机械打字机的字锤在快速连击时容易卡住。QWERTY布局刻意把常用字母对分散到键盘两侧，以减少相邻字锤碰撞的概率。
...
这就是路径依赖与锁定效应：**早期的选择——往往是偶然的、基于当时特殊条件的选择——通过不断积累的切换成本和[网络效应](/model/network-effects)，将整个系统锁定在一条特定的发展路径上，即使这条路径已不再是最优的。**
...
这解释了为什么路径依赖在[复杂系统](/model/complex-systems)中比在简单系统中更强。一个独立工具的切换成本可能很低，但一个嵌入了庞大生态系统的技术标准，其切换成本可能高到没有任何单一参与者有动力去承担。

**第三个机制：认知锁定。** 除了经济层面的切换成本，人类的认知也会被路径锁定。一旦你学会了用特定的方式理解一个系统——特定的心智模型、特定的分析框架、特定的术语——切换到另一种理解方式需要付出巨大的认知努力。芒格所说的“拿着锤子的人看什么都像钉子”（[铁锤人倾向](/model/man-with-a-hammer-syndrome)），本质上就是一种认知层面的路径依赖：你最先学会的工具，会锁定你看待问题的方式。
...
芒格和巴菲特对此心知肚明。他们投资的很多企业——从可口可乐到苹果——都拥有强大的锁定效应。芒格用“[护城河（Moat）](/model/moat)”来描述企业的持久竞争优势，而路径依赖和锁定正是最深、最宽的护城河之一。一旦消费者的习惯、企业的基础设施、行业的标准全部围绕你的产品建立起来，竞争对手面对的不是一家公司，而是一整个生态系统的惯性。
...
3. **警惕自己的认知路径依赖。** 你最初分析一家企业时形成的看法，会不会锁定了你对它的后续判断？[避免不一致性倾向](/model/inconsistency-avoidance-tendency)和路径依赖在心理层面是近亲——一旦你形成了一个观点，你的大脑会自动过滤掉不一致的信息，强化一致的信息，把一个偶然形成的初始判断锁定为牢不可破的信念。芒格对此的解药是[逆向思维](/model/inversion)——定期强迫自己从反面论证。
...
第一种错误是**高估理性选择的力量**。“更好的产品一定会胜出”——不一定。在存在强路径依赖的市场中，先发优势和生态锁定可能比产品质量更重要。无数技术上更优秀的产品败给了锁定效应更强的竞争对手。

第二种错误是**低估变革的可能性**。“现有的锁定不可打破”——也不一定。每一代技术范式转换都重置了上一代的锁定格局。大型机的锁定被PC打破，PC的锁定被移动互联网打破。锁定是强大的，但它不是永恒的。
...
- [护城河（Moat）](/model/moat) — 锁定效应是最重要的护城河来源之一
- [转换成本](/model/switching-costs) — 转换成本是路径依赖在商业中最直接的表现
- [网络效应](/model/network-effects) — 网络效应是产生正反馈和路径依赖的核心机制
- [飞轮效应](/model/flywheel-effect) — 飞轮效应通过正反馈循环创造路径依赖
- [规模效应与反效应](/model/scaling-effects-diseconomies-in-engineering) — 规模经济与路径依赖相互强化
- [铁锤人倾向](/model/man-with-a-hammer-syndrome) — 认知层面的路径依赖：工具锁定思维方式
- [避免不一致性倾向](/model/inconsistency-avoidance-tendency) — 信念层面的路径依赖：初始判断锁定后续认知
- [沉没成本谬误](/model/sunk-cost-fallacy) — 沉没成本是维持路径锁定的心理力量之一
- [被剥夺超级反应倾向](/model/deprival-superreaction-tendency) — 对已有资产的过度评估强化了切换成本
- [跨学科思维](/model/multidisciplinary-approach) — 对抗认知路径依赖的核心策略
- [可维护性设计](/model/design-for-maintainability) — 良好的可维护性设计降低路径切换的成本
- [逆向思维](/model/inversion) — 打破认知路径依赖的有效方法
