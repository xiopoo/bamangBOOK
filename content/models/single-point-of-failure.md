...
sourceUrl: "https://mungermodels.com/models/single-point-of-failure"
---
...
**第三，[非线性后果](/model/nonlinear-consequences)。** 单点故障的特征是：触发事件的规模与后果的规模完全不成比例。一条错误的命令导致1.5亿美元的损失。一个橡胶密封圈的失效导致七条生命的丧失。这种极端的不对称性是单点故障区别于一般性风险的核心特征。
...
- [冗余备份系统](/model/redundancy) — 消除单点故障的核心手段：为关键环节构建备份
- [故障模式分析](/model/failure-mode-analysis) — FMEA是识别单点故障的系统化方法
- [断裂点](/model/breakpoints) — 单点故障就是系统的断裂点所在
- [护城河（Moat）](/model/moat) — 依赖单一竞争优势的企业本质上有战略层面的单点故障
- [安全边际](/model/margin-of-safety) — 安全边际为单点故障提供缓冲空间
- [紧耦合与松耦合](/model/tight-coupling-loose-coupling) — 紧耦合系统中的单点故障传导更快、后果更严重
- [脆弱性与反脆弱性](/model/fragility-antifragility) — 单点故障是系统脆弱性的典型表现
- [逆向思维](/model/inversion) — 识别单点故障的思维方式本质上就是逆向思考
