...
sourceUrl: "https://mungermodels.com/models/off-balance-sheet-contingent-liabilities"
---
...
或有负债（Contingent Liabilities）是另一类在报表上看不到或很难看到的风险。与表外负债不同，或有负债的特点是**不确定性**——你不知道它是否会发生，不知道什么时候发生，不知道金额多大。
...
芒格对此的应对不是试图精确估算，而是**在估值中留出足够的[安全边际](/model/margin-of-safety)**。如果你必须做出一个精确的或有负债估计才能证明某只股票值得买，那它可能不值得买。真正好的投资是那些——即使所有或有负债都以最坏情景兑现——仍然提供足够回报的投资。
...
- [会计作为商业语言及其局限](/model/accounting-as-language-its-limits) — 表外负债是会计语言最大的盲区之一
- [现金流量vs利润](/model/cash-flow-vs-earnings) — 表外负债不影响利润表但消耗真实现金，现金流分析是穿透表外操作的关键工具
- [安全边际](/model/margin-of-safety) — 面对不可量化的表外风险，安全边际是最后的防线
- [激励机制](/model/incentives-economic-view) — 管理层有系统性的激励去最小化表外负债的披露
- [逆向思维](/model/inversion) — “这家公司可能有哪些我在报表上看不到的义务？”是芒格式逆向思考的核心问题
- [检查清单方法](/model/checklist-method) — 系统性检查表外项目需要一份清单，因为它们太容易被遗漏
