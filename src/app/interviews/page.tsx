...
export default function InterviewsPage({ searchParams }: PageProps) {
  const personId = searchParams.person
  const documents = getDocuments('interviews', personId)
  ...
  return (
    <PageContainer maxWidth="7xl">
      <PageHeader
        title={`${getCategoryIcon('interviews')} ${getCategoryTitle('interviews')}${currentPerson ? ` - ${currentPerson.name}` : ''}`}
        subtitle={currentPerson ? `${currentPerson.name}的访谈实录，深入了解投资大师的思考方式` : `巴菲特和芒格的访谈实录，深入了解投资大师的思考方式`}
        backHref="/"
        backLabel="返回首页"
        sticky
      />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-10">
        <StatBadge icon="🎙️" count={`${totalCount}篇`} label="访谈" sub="媒体专访" />
        {firstYear && lastYear && (
          <StatBadge icon="📅" count={`${lastYear - firstYear + 1}年`} label="时间跨度" sub={`${firstYear}-${lastYear}`} />
        )}
        <StatBadge icon="📝" count={`${(totalWords / 10000).toFixed(1)}万`} label="总字数" sub="访谈内容" />
        <StatBadge icon="💬" count="深度对话" label="形式" sub="问答交流" />
      </div>
            ...
            <section key={decade.label}>
              ...
              <div className="space-y-3">
                {decadeDocs.map((doc) => (
                  <Link
                    key={doc.fileName}
                    href={`/interviews/${encodeURIComponent(doc.fileName)}`}
