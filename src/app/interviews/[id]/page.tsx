...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  if (!doc) return { title: '访谈未找到', robots: { index: false } }
  return {
    title: `${doc.title} · 访谈档案`,
    description: summarizeContent(doc.content),
    alternates: { canonical: `/interviews/${encodeURIComponent(fileName)}` },
      ...
      <header className="bg-bg-card dark:bg-dark-card border-b border-primary/10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <Link href="/interviews" className="text-sm text-primary hover:text-primary-light transition-colors mb-2 inline-flex items-center gap-1">
                ← 返回{getCategoryTitle('interviews')}列表
              </Link>
              ...
              <p className="text-sm text-text-muted dark:text-dark-muted">
                {getCategoryIcon('interviews')} {getCategoryTitle('interviews')}
                {doc.year && ` · ${doc.year}年`}
              </p>
            </div>
