...
export default function ReadingMetadata({ person, year, contentType, sourceLabel, sourceUrl, status = '编辑整理', completeness, reviewedAt, readMinutes }: ReadingMetadataProps) {
  return <div className="reading-metadata" aria-label="阅读资料信息">
    ...
    <div className="reading-metadata__secondary">{sourceLabel && <span>来源 · {sourceUrl ? <a href={sourceUrl} target="_blank" rel="noopener noreferrer">{sourceLabel} ↗</a> : sourceLabel}</span>}<span className="reading-status">内容状态 · {status}</span>{completeness && completeness !== '未知' && <span>完整度 · {completeness}</span>}{reviewedAt && <span>最近修订 · {reviewedAt}</span>}</div>
  </div>
}
