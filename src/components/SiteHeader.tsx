...
const navGroups = [
  ...
  { label: '公司研究', activePrefixes: ['/business-history', '/companies'], links: [
    ...
    { href: '/business-history', label: '商业主题', meta: '从行业与商业史进入' },
  ] },
...
export default function SiteHeader() {
  ...
  useEffect(() => {
    ...
    const escape = (event: KeyboardEvent) => { if (event.key === 'Escape') setOpenGroup(null) }
  ...
  return <header ref={headerRef} className="archive-masthead">
    <div className="archive-masthead__inner">
      ...
      <div className="archive-masthead__tools">
        ...
        <button type="button" className="archive-menu-toggle" onClick={() => setMobileOpen(value => !value)} aria-expanded={mobileOpen} aria-controls="mobile-navigation" aria-label={mobileOpen ? '关闭菜单' : '打开菜单'}>{mobileOpen ? <X size={22} /> : <Menu size={22} />}</button>
      </div>
    </div>
